...
void CrtSurfData(){
	...
	for(int ii=0;ii<vstcode.size();ii++){
		...
		strncpy(stsurfdata.ddatetime,strddatetime,14);	//时间 格式yyyymmddhh24miss
...
bool CrtSurfFile(const char* outpath,const char* datafmt){
	...
	if(strcmp(datafmt,"json")==0){
        File.Fprintf("{\"obtid\":\"%s\",\"ddatetime\":\"%s\",\"t\":\"%.1f\",\"p\":\"%.1f\",\"u\":\"%d\",\"wd\":\"%d\",\"wf\":\"%.1f\",\"r\":\"%.1f\",\"vis\":\"%.1f\"}",vsurfdata[ii].obtid,vsurfdata[ii].ddatetime,\
                                vsurfdata[ii].t/10.0,vsurfdata[ii].p/10.0,vsurfdata[ii].u,vsurfdata[ii].wd,vsurfdata[ii].wf/10.0,\
                                vsurfdata[ii].r/10.0,vsurfdata[ii].vis/10.0);
...
int main(int argc, char *argv[]){
	...
	if (argc!=5&&argc!=6)
	{
		std::cout<<"Using:./crtsurfdata inifile outpath logfile datafmt [datatime]"<<std::endl;
		std::cout<<"Example:/home/project/myidc/bin/crturfdata /home/project/myidc/ini/stcode.ini /tmp/surfdata /log/idc/crtsurfdata.log xml,json,csv 20220710123000"<<std::endl;
		...
		std::cout<<"datatime 可选项，表示生成指定时间的数据"<<std::endl;
		return -1;
	}
